      ...
      {isExpanded && (
        <div className="px-4 py-3 border-t dark:border-gray-700 bg-gray-50 dark:bg-gray-800/50 space-y-3">
          ...
          {log.logs && log.logs.length > 0 && (
            <div>
              <span className="text-xs font-medium text-gray-600 dark:text-gray-300 block mb-1">Logs</span>
                ...
                {log.logs.map((entry, i) => (
                  <div 
                    ...
                    className={cn(
                      ...
                      entry.level === 'error' && "bg-red-50 dark:bg-red-900/30 text-red-700 dark:text-red-400",
                      entry.level === 'warn' && "bg-yellow-50 dark:bg-yellow-900/30 text-yellow-700 dark:text-yellow-400",
                      entry.level === 'info' && "bg-blue-50 dark:bg-blue-900/30 text-blue-700 dark:text-blue-400",
                      entry.level === 'debug' && "bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300"
                    )}
                  >
                    <span className="opacity-60">[{entry.level.toUpperCase()}]</span> {entry.message}
                  </div>
                ))}
              </div>
            </div>
...
export default function TraceLogsPage() {
  ...
  const { data: executionsData, isLoading, refetch, isFetching } = useQuery({
    ...
    queryFn: () => executionsApi.listByWorkflow(id!),
    enabled: !!id,
          ...
          <div className="flex-1 overflow-y-auto">
            {filteredExecutions.length === 0 ? (
              <div className="p-8 text-center text-gray-500 dark:text-gray-400">
                ...
                <p className="text-sm">Run the workflow to see trace logs</p>
              </div>
            ) : (
          ...
          {selectedExecution ? (
              ...
              <div className="bg-white dark:bg-gray-800 border-b dark:border-gray-700 px-6 py-4">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <Link
                      to={`/workflows/${id}/executions/${selectedExecution.id}`}
                      className="p-2 text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-lg"
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-6">
                ...
                {selectedExecution.metadata && Object.keys(selectedExecution.metadata).length > 0 && (
                  <div className="bg-white dark:bg-gray-800 rounded-xl border dark:border-gray-700 shadow-sm overflow-hidden">
                    ...
                    <pre className="p-4 text-sm overflow-auto font-mono text-gray-800 dark:text-gray-200">
            ...
            <div className="flex-1 flex items-center justify-center text-gray-500 dark:text-gray-400">
              <div className="text-center">
                ...
                <p className="text-sm">to view trace logs</p>
              </div>
            </div>
          )}
        </div>
